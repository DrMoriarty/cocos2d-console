...
class AmazonBuilder(object):
    ...
    def __init__(self, verbose, app_amazon_root, no_res, proj_obj, ndk_mode, app_abi, gradle_support_ndk=False):
        self._verbose = verbose

        self.app_amazon_root = app_amazon_root
        self._no_res = no_res
        ...
        self.ant_root = None
        self.sign_prop_file = os.path.join(self.app_amazon_root, 'app', "gradle.properties")
    ...
    def _parse_cfg(self):
        ...
        self.key_store_str = AmazonBuilder.GRADLE_KEY_STORE
        self.key_alias_str = AmazonBuilder.GRADLE_KEY_ALIAS
        self.key_store_pass_str = AmazonBuilder.GRADLE_KEY_STORE_PASS
        self.key_alias_pass_str = AmazonBuilder.GRADLE_KEY_ALIAS_PASS

        if self.gradle_support_ndk:
            return

        self.cfg_path = os.path.join(self.app_amazon_root, BUILD_CFIG_FILE)
        try:
        ...
        move_cfg = {}
        self.key_store = None
        if cfg.has_key(AmazonBuilder.CFG_KEY_STORE):
            self.key_store = cfg[AmazonBuilder.CFG_KEY_STORE]
            move_cfg[self.key_store_str] = self.key_store
            del cfg[AmazonBuilder.CFG_KEY_STORE]

        self.key_store_pass = None
        if cfg.has_key(AmazonBuilder.CFG_KEY_STORE_PASS):
            self.key_store_pass = cfg[AmazonBuilder.CFG_KEY_STORE_PASS]
            move_cfg[self.key_store_pass_str] = self.key_store_pass
            del cfg[AmazonBuilder.CFG_KEY_STORE_PASS]

        self.alias = None
        if cfg.has_key(AmazonBuilder.CFG_KEY_ALIAS):
            self.alias = cfg[AmazonBuilder.CFG_KEY_ALIAS]
            move_cfg[self.key_alias_str] = self.alias
            del cfg[AmazonBuilder.CFG_KEY_ALIAS]

        self.alias_pass = None
        if cfg.has_key(AmazonBuilder.CFG_KEY_ALIAS_PASS):
            self.alias_pass = cfg[AmazonBuilder.CFG_KEY_ALIAS_PASS]
            move_cfg[self.key_alias_pass_str] = self.alias_pass
            del cfg[AmazonBuilder.CFG_KEY_ALIAS_PASS]
    ...
    def update_project(self, android_platform):
        if self.gradle_support_ndk:
            ...
            self._write_local_properties(self.app_amazon_root)
            return

        manifest_path = os.path.join(self.app_amazon_root, 'app')
        ...
        # copy the local.properties to the app_amazon_root
        file_name = 'local.properties'
        src_path = os.path.normpath(os.path.join(manifest_path, file_name))
        dst_path = os.path.normpath(os.path.join(self.app_amazon_root, file_name))
    ...
    def do_ndk_build(self, ndk_build_param, build_mode, compile_obj):
        ...
        ndk_work_dir = os.path.join(self.app_amazon_root, 'app')
        reload(sys)
        ...
        for cfg_path in self.ndk_module_paths:
            if cfg_path.find("${COCOS_X_ROOT}") >= 0:
                ...
                module_paths.append(os.path.normpath(os.path.join(self.app_amazon_root, cfg_path)))
    ...
    def gradle_build_apk(self, build_mode, android_platform, compile_obj):
        ...
        check_file = os.path.join(self.app_amazon_root, 'app', 'build.gradle')
        f = open(check_file)
        lines = f.readlines()
        f.close()
        ...
        if cocos.os_is_win32():
            gradle_path = os.path.join(self.app_amazon_root, 'gradlew.bat')
        else:
            gradle_path = os.path.join(self.app_amazon_root, 'gradlew')
        ...
        if self.gradle_support_ndk:
            add_props = {
                AmazonBuilder.GRADLE_PROP_NDK_MODE: self.ndk_mode
            }
            if android_platform:
                ...
                if match:
                    add_props[AmazonBuilder.GRADLE_PROP_TARGET_VERSION] = (int)(match.group(1))

            if self.app_abi:
                add_props[AmazonBuilder.GRADLE_PROP_APP_ABI] = ':'.join(self.app_abi.split(' '))

            if self._project._is_script_project():
                if compile_obj._compile_script:
                    add_props[AmazonBuilder.GRADLE_PROP_COMPILE_SCRIPT] = 1
                else:
                    add_props[AmazonBuilder.GRADLE_PROP_COMPILE_SCRIPT] = 0

            if self._project._is_lua_project():
                if compile_obj._lua_encrypt:
                    add_props[AmazonBuilder.GRADLE_PROP_LUA_ENCRYPT] = 1
                    add_props[AmazonBuilder.GRADLE_PROP_LUA_ENCRYPT_KEY] = compile_obj._lua_encrypt_key
                    add_props[AmazonBuilder.GRADLE_PROP_LUA_ENCRYPT_SIGN] = compile_obj._lua_encrypt_sign

            for key in add_props.keys():
                cmd += ' -P%s=%s' % (key, add_props[key])

        self._run_cmd(cmd, cwd=self.app_amazon_root)
    ...
    def _get_build_type(self, param_of_appabi):
        ...
        applicationmk_path = os.path.join(self.app_amazon_root, "app/jni/Application.mk")
    ...
    def do_build_apk(self, build_mode, no_apk, output_dir, custom_step_args, android_platform, compile_obj):
        assets_dir = os.path.join(self.app_amazon_root, "app", "assets")
        project_name = None
        setting_file = os.path.join(self.app_amazon_root, 'settings.gradle')
        ...
        if project_name is None:
            ...
            project_name = 'app'
        gen_apk_folder = os.path.join(self.app_amazon_root, 'app/build/outputs/apk')
    ...
    def _gather_sign_info(self):
        ...
        while True:
            ...
            if not os.path.isabs(inputed):
                start_path = os.path.join(self.app_amazon_root, 'app')
    ...
    def _copy_resources(self, custom_step_args, assets_dir):
        app_amazon_root = self.app_amazon_root
        res_files = self.res_files

        # remove app_amazon_root/assets if it exists
        if os.path.isdir(assets_dir):
            shutil.rmtree(assets_dir)

        # generate parameters for custom steps
        target_platform = cocos_project.Platforms.AMAZON
        ...
        for cfg in res_files:
            cocos.copy_files_with_config(cfg, app_amazon_root, assets_dir)
